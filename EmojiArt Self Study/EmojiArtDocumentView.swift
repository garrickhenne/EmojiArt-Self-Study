...
// Step 3: Renamed this from "ContentView" to "EmojiArtDocumentView" (command-click then rename), step 4 is in scene delegate file
struct EmojiArtDocumentView: View {
    
    // Step 2: add ObservedObject var of viewmodel
    @ObservedObject var document: EmojiArtDocument
    
    // State are almost always private, this var is binded with PaletteChooser
    ...
    var body: some View {
        VStack {
            HStack {
                // Notice that $chosenPalette is used, means its a binding to the chosenPalette
                PaletteChooser(document: document, chosenPalette: $chosenPalette)
                ScrollView(.horizontal) {
                    HStack {
                        // id of self makes each emoji in the string identifiable. \.self is a "keypath"
